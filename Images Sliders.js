...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const sliderWrapper = document.querySelector(".global-zoom-slider");
    ...
    if (!postBody || !sliderWrapper) {
        ...
        console.error(
            "Required .post-body or .global-zoom-slider element not found."
        );
        return;
    }
    const media = [];
    ...
    const mainWrapper = document.getElementById("globalMainSlider");
    const thumbWrapper = document.getElementById("globalThumbSlider");
    const fullscreenMediaContainer = document.querySelector(
            ...
            mainSlide.innerHTML = `
                        <div class="zoom-container" data-index="${index}">
                            ...
                            <div class="controls">
                                ...
                                <div class="zoom-value" aria-live="polite">${zoomSettings.level.toFixed(
                    1
                )}x</div>
                            </div>
                        </div>`;
    ...
    const initializeSwiper = () => {
        thumbSwiper = new Swiper(".global-thumb-slider", {
            slidesPerView: "auto",
            spaceBetween: 10,
            freeMode: !0,
            watchSlidesProgress: !0,
            navigation: {
                nextEl: ".global-slider-nav.next",
                prevEl: ".global-slider-nav.prev",
            },
        });
        mainSwiper = new Swiper(".global-main-slider", {
            loop: media.length > 1,
    ...
    const initializeFullscreen = () => {
        const fullscreenOverlay = document.querySelector(".global-fullscreen");
        const fullscreenClose = document.querySelector(".global-fullscreen-close");
        ...
        const updateFullscreenMedia = (index) => {
            ...
            const imgElement = document.querySelector(".global-fullscreen-img");
        ...
        const updateFullscreenZoom = () => {
            const mediaElement = document.querySelector(".global-fullscreen-img");
            if (mediaElement) {
        ...
        const endDrag = () => {
            if (isDragging) {
                isDragging = !1;
                mediaElement.classList.remove("dragging");
            }
        };
    ...
    waitForSwiper((swiperInstance) => {
        try {
            ...
            const defaults = {
                ...
                currentPrice: (
                    raw.match(/\[data-current-price="(.*?)"\]/)?.[1] || ""
                ).trim(),
            ...
            imageAltMap.forEach((e) => {
                if (e.index !== undefined) e.index = Number(e.index);
            });
                ...
                return {
                    oldPrice:
                        v.oldPrice !== undefined &&
                            v.oldPrice !== null &&
                            String(v.oldPrice).trim() !== ""
                            ? v.oldPrice
                            : defaults.oldPrice,
                    currentPrice:
                        v.currentPrice !== undefined &&
                            v.currentPrice !== null &&
                            String(v.currentPrice).trim() !== ""
                            ? v.currentPrice
            ...
            function updateDisplayWithData(data) {
                document.querySelectorAll(".post[data-post-url]").forEach((post) => {
                    ...
                    post.querySelectorAll(".saved-product-sale").forEach((div) => {
                        ...
                        const valid =
                            typeof sale === "string" && sale.endsWith("%") && !isNaN(percent);
            ...
            function updateDisplay(variantName) {
                const variant = variantName
                    ? imageAltMap.find(
                        (v) =>
                            v.name &&
                            v.name.toLowerCase() === String(variantName).toLowerCase()
                    )
                    : null;
                const merged = getMergedValues(variant);
                console.debug(
                    "VariantSync: updateDisplay for",
                    variantName,
                    "merged:",
                    merged
                );
            ...
            function updateVariantSelectionUI(variantNameToSelect) {
                const nameLower = variantNameToSelect
                    ? variantNameToSelect.toLowerCase()
                    : null;
                document
                    .querySelectorAll(".variant-group [data-label]")
                    .forEach((btn) => {
            ...
            document.querySelectorAll(".variant-group [data-label]").forEach((el) => {
                el.addEventListener("click", function () {
                    ...
                    const match = imageAltMap.find(
                        (v) => v.name && v.name.toLowerCase() === variantName.toLowerCase()
                    );
                    if (match) {
                        ...
                        console.debug(
                            "VariantSync: clicked variant not found in imageAltMap:",
                            variantName
                        );
                    }
            ...
            swiperInstance.on("slideChange", () => {
                ...
                const activeVariant = imageAltMap.find(
                    (v) => v.index === activeRealIndex && v.name
                );
                console.debug(
                    "VariantSync: slideChange activeRealIndex=",
                    activeRealIndex,
                    "activeVariant=",
                    activeVariant
                );
            ...
            const initialVariant = imageAltMap.find(
                (v) => v.index === initialReal && v.name
            );
            console.debug(
                "VariantSync: initialRealIndex=",
                initialReal,
                "initialVariant=",
                initialVariant
            );
...
document.addEventListener("DOMContentLoaded", function () {
    document
        .querySelectorAll(".homepage-slider-wrapper")
        .forEach((wrapper, index) => {
            // --- Part 1: Gather Essential Elements and Data ---
            const postId = wrapper.getAttribute("data-post-id");
            const bodyHtml = wrapper.querySelector(
                ".homepage-hidden-body"
            )?.innerHTML;
            const mainWrapper = wrapper.querySelector(
                ".homepage-main-slider .swiper-wrapper"
            );
            const thumbWrapper = wrapper.querySelector(
                ".homepage-thumb-slider .swiper-wrapper"
            );
            ...
            if (!postId || postId.includes("{") || postId.includes("data:")) {
                ...
                console.warn(
                    `Slider #${index + 1
                    }: Invalid or missing 'data-post-id'. LocalStorage disabled.`
                );
            } else {
            ...
            const formatPrice = (value) => {
                ...
                const decimalMatch = numberPart.match(/^(\d+)(\.\d{1,2})?$/);
                if (!decimalMatch) return value.trim();
                const formattedInteger = decimalMatch[1].replace(
                    /\B(?=(\d{3})+(?!\d))/g,
                    ","
                );
                return symbol
                    ? `${symbol} ${formattedInteger}${decimalMatch[2] || ""}`
                    : `${formattedInteger}${decimalMatch[2] || ""}`;
            };

            productData.defaults = {
                oldPrice: cleanPriceString(
                    bodyHtml.match(/\[data-old-price="(.*?)"\]/)?.[1] || ""
                ),
                currentPrice: cleanPriceString(
                    bodyHtml.match(/\[data-current-price="(.*?)"\]/)?.[1] || ""
                ),
                stock: (
                    bodyHtml.match(/\[data-stock="(.*?)"\]/)?.[1] || "In Stock"
                ).trim(),
            ...
            if (isIdValid) {
                try {
                    ...
                    console.error(`Error saving product ${postId} to localStorage:`, e);
                }
            }
            ...
            setTimeout(() => {
                const thumbSwiper = thumbWrapper
                    ? new Swiper(wrapper.querySelector(".homepage-thumb-slider"), {
                        slidesPerView: "auto",
                ...
                const mainSwiper = new Swiper(
                    wrapper.querySelector(".homepage-main-slider"),
                    {
                        effect: "fade",
                ...
                const variantContainer = parent.querySelector(
                    `.homepage-view-variants[data-post-id="${postId}"]`
                );
                const allowedBadges = [
                    "Hot",
                    "New",
                ...
                function updateDisplay(variantName) {
                    const variantData = variantName
                        ...
                        : undefined;
                    const oldPrice =
                        variantData?.oldPrice || productData.defaults.oldPrice;
                    const currentPrice =
                        variantData?.currentPrice || productData.defaults.currentPrice;
                    ...
                    parent.querySelectorAll(".saved-stock").forEach((div) => {
                        ...
                        if (stock === "Out of Stock" || stock === "0")
                            ...
                            (div.textContent = "In Stock"), div.classList.add("in-stock");
                ...
                function updateVariantSelectionUI(variantName) {
                    ...
                    variantContainer
                        .querySelectorAll("span[data-label]")
                        .forEach((btn) => {
                            const btnLabel = btn
                ...
                const initialVariantName =
                    indexToVariantNameMap[mainSwiper.activeIndex];
